# train_multiGPU_fix_RESUME_CORRECTED.py

# COMPLETE PRODUCTION-READY VERSION - FIXED RESUME ISSUE
...
# ✅ FIXED: Resume now uses same checkpoint directory instead of creating new ones
...
def extract_timestamp_from_path(checkpoint_path):
    """Extract timestamp from checkpoint path if resuming"""
    # Expected format: .../hff-l=0.3-e=200-.../... or .../ABLATION_FDD-hff-l=0.3-.../
    # Timestamp is usually at the end: YYYY-MM-DD_HH-MM-SS
    pattern = r'(\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2})'
    matches = re.findall(pattern, checkpoint_path)
    if matches:
        return matches[-1]  # Get the last (most recent) timestamp
    return None
...
if __name__ == '__main__':
    ...
    # ===== FIXED: Determine timestamp BEFORE creating logger =====
    # If resuming, extract timestamp from checkpoint path; otherwise, generate new one
    if args.resume_checkpoint is not None:
        extracted_timestamp = extract_timestamp_from_path(args.resume_checkpoint)
        timestamp = extracted_timestamp if extracted_timestamp else datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        if is_main_process:
            print(f"[RESUME] Using existing experiment directory (timestamp: {timestamp})")
    else:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        if is_main_process:
            print(f"[NEW] Starting new experiment (timestamp: {timestamp})")

    # Create logs directory and set up logger
    ...
    if is_main_process:
        ...
        print(f"Resume: {args.resume_checkpoint is not None}")
    ...
    if is_main_process:
        ...
        wandb.config.update({'resume': args.resume_checkpoint is not None})
    ...
    # ===== FIXED: Build path with timestamp determined above =====
    path_trained_models = (
    ...
    path_trained_models = (
        ...
        + str(args.input2)
        + '-'
        + timestamp  # Use the determined timestamp, not a new one
    )
    ...
    if is_main_process:
        print(f"Checkpoint directory: {path_trained_models}")
    # ===== END FIXED =====
