# attention_vis_WITH_MECHANISTIC.py
# COMPLETE VERSION WITH MECHANISTIC AI FEATURE IMPORTANCE FUNCTION

# Copy this entire file to replace your attention_vis.py
...
class EnhancedFDCAAttentionVisualizer:
    """Enhanced FDCA attention extraction with 3D volume handling and mechanistic interpretability"""
    ...
    def __init__(self, device='cuda', save_dir='results/figures/xai/attention', dpi=600):
        ...
        self.attention_maps = {}
        self.hooks = []
        self.activations_cache = {}  # ← KEY: Store activations for mechanistic analysis
    ...
    def compute_feature_importance(self, activations: Dict) -> Dict:
        """
        ===== MECHANISTIC AI FUNCTION =====
        Compute which features (neurons) are most important
        
        The importance of a feature is measured by its variance:
        - High variance = selective feature (fires only on specific inputs)
        - Low variance = generic feature (fires everywhere)
        """
        importance_scores = {}
        
        for layer_name, act_array in activations.items():
            if act_array.ndim >= 2:
                # Compute variance across all dimensions except channels
                # This measures how much each channel/feature varies across the input
    ...
    def aggregate_attention_maps(self, attention_maps: Dict) -> torch.Tensor:
        ...
        for _, attn in attention_maps.items():
            if attn.ndim == 5:  # (B, C, D, H, W)
                attn_spatial = attn.mean(dim=(1, 2))
            elif attn.ndim == 4:  # (B, C, H, W)
                attn_spatial = attn.mean(dim=1)
            else:
                continue
...
class EnhancedSegmentationGradCAM:
    ...
    def generate_cam(self, input_tensor: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        if output_main.ndim == 5:
            target_output = output_main[:, target_class, ...]
        elif output_main.ndim == 4:
            target_output = output_main[:, target_class, ...]
        else:
        ...
        if eigen_smooth:
            for i in range(cam.shape[0]):
                if cam[i].ndim == 3:
                    cam[i] = gaussian_filter(cam[i][cam[i].shape[0]//2], sigma=1.5)
                else:
