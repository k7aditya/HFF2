# train_multiGPU_ablation.py

# COMPLETE PRODUCTION-READY VERSION WITH ABLATION STUDY SUPPORT
...
# ✅ ABLATION STUDY: FDD, ALC, FDCA components can be enabled/disabled via argparse
...
from model.HFF_MobileNetV3_fixed_ablation import HFFNet
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='HFFNet Training with Ablation Study Support')

    # ==================== ABLATION STUDY ARGUMENTS ====================
    parser.add_argument('--enable_fdd', action='store_true', default=False,
                        help='Enable FDD (Frequency Domain Decomposition) component')
    parser.add_argument('--enable_alc', action='store_true', default=False,
                        help='Enable ALC (Adaptive Laplacian Convolution) component')
    parser.add_argument('--enable_fdca', action='store_true', default=False,
                        help='Enable FDCA (Frequency Domain Cross-Attention) component')
    # ==================================================================
    ...
    # ===== Create ablation experiment name =====
    ablation_config = []
    if args.enable_fdd:
        ablation_config.append('FDD')
    if args.enable_alc:
        ablation_config.append('ALC')
    if args.enable_fdca:
        ablation_config.append('FDCA')

    if not ablation_config:
        ablation_name = 'baseline_no_components'
    else:
        ablation_name = '+'.join(ablation_config)
    ...
    if is_main_process:
        ...
        print(f"  - FDD (Frequency Domain Decomposition): {'ENABLED' if args.enable_fdd else 'DISABLED'}")
        print(f"  - ALC (Adaptive Laplacian Convolution): {'ENABLED' if args.enable_alc else 'DISABLED'}")
        print(f"  - FDCA (Frequency Domain Cross-Attention): {'ENABLED' if args.enable_fdca else 'DISABLED'}")
        print("="*80 + "\n")

    if is_main_process:
        wandb.init(
            project=f'ABLATION_{ablation_name}_lr={args.lr}_epochs={args.num_epochs}_step={args.step_size}_gamma={args.gamma}_unsup={args.unsup_weight}_warmup={args.warm_up_duration}_{args.dataset_name}_{args.class_type}',
            name=f'{ablation_name}_experiment'
        )
        wandb.config.update(args)
        wandb.config.update({
            'ablation_config': ablation_name,
            'enable_fdd': args.enable_fdd,
            'enable_alc': args.enable_alc,
            'enable_fdca': args.enable_fdca
        })
    ...
    # ===== INITIALIZE MODEL WITH ABLATION FLAGS =====
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for i, data in enumerate(tqdm(loaders['train'], disable=not is_main_process)):
            ...
            # ALC Component - only compute reg_loss if ALC is enabled
            if args.enable_alc and model_to_update.laplacian_target is not None:
